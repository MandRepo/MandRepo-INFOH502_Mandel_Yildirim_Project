...
uniform float quadratic;
 ...
 void main() {   
	...
	vec3 basecolor = texture(lampSampler, texCoords).rgb;
	
	vec3 N = normalize(fragNormal);

	vec3 ambient = ambient_strength * basecolor;
	
	vec3 light_diff  = vec3(0.0f);
	...
	for (int i=0; i<lamp_count; i++){
		...
		light_diff += diffuse *attenuation;
	}


	FragColor =  vec4(ambient+light_diff, 1.0 );
 }   
